...
class PersonCl {
    ...
    set fullName(name) //same as property name so we get the value from object property name ->fullName
    {
        console.log(name);
        if (name.includes(' ')) this._fullName = name; //new property set same name diye so use underscore_
        else alert(`${name} is not a full name!`);
